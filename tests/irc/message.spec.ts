...
import { DefaultMessage, parseMessage, DefaultText } from '../../src/irc/message'

describe('Message', () => {
  describe('Message', () => {
    it('isChannel true', () => {
      const message = new DefaultMessage('', '', ['#test'], new DefaultText(''))
    ...
    it('isChannel not enough params', () => {
      const message = new DefaultMessage('', '', [], new DefaultText(''))
      expect(message.isChannel()).to.be.false
    })
  })
  ...
  describe('Text', () => {
    it('command single word', () => {
      ...
      const text = new DefaultText(command)
    ...
    it('command multiple words', () => {
      ...
      const text = new DefaultText(`${command} bla bla bla`)
      expect(text.command()).to.be.equal(command)
    })
  })
